...
const fixRelativeInstructions = switch (builtin.cpu.arch) {
    .x86_64 => any.x86_64.relative_rip_instructions.fix,
    else => @compileError("TODO: Support more archs"),
};
const detectFuncEnd = switch (builtin.cpu.arch) {
...
pub fn init(allocator: Allocator) !Self {
    const disassmbler = try disasm.createManagedCapstone(builtin.cpu.arch, .{ .detail = true });
...
pub fn attach(self: *Self, target: usize, detour: usize) !Attached {
    ...
    if (!get_put_res.found_existing) {
        ...
        _ = try emitJmp(@intFromPtr(jmp_entry.detour_jmp.ptr), detour, absolute_jmp_size);
        _ = try emitJmp(@intFromPtr(jmp_entry.next_jmp.ptr), @intFromPtr(ret_trampoline.ptr), absolute_jmp_size);
        ...
        if (ending_ins) |end_ins| {
            ...
            _ = try emitJmp(@intFromPtr(ret_trampoline.ptr) + jmp_back_write_offset, jmp_back_original, null);
        }
        ...
        _ = try emitJmp(@intFromPtr(new_jmp_entry.detour_jmp.ptr), detour, absolute_jmp_size);
        _ = try emitJmp(@intFromPtr(last_jmp_entry.next_jmp.ptr), @intFromPtr(new_jmp_entry.detour_jmp.ptr), absolute_jmp_size);
        new_jmp_entry.flush();
...
pub fn detach(self: *Self, target: usize, detour: usize) !void {
    ...
    if (index == 0) {
        ...
        _ = try emitJmp(target, @intFromPtr(next_entry.detour_jmp.ptr), info.overwritten_size);
...
pub fn emitJmp(to: usize, target: usize, overwrite_bytes: ?usize) !usize {
    ...
    switch (builtin.cpu.arch) {
        .x86_64 => {
            ...
            any.x86_64.writeJmpInstruction(&ins_buf, target);
            for (0..ins_len - any.x86_64.absolute_jmp_size) |i| {
                ins_buf[any.x86_64.absolute_jmp_size + i] = 0x90; // nop
            }
        },
...
test "usage x86_64" {
    ...
    const code_jump_back_address = @intFromPtr(code[absolute_jmp_size..].ptr);
